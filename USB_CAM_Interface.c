...
static int camera_frame(camera_t* camera, struct timeval timeout) {
    fd_set fds;
    FD_ZERO(&fds);
...
#if 0
static int saveFile(const char * name, void* buf, int size)
{
...
static int getYFromYUYV(char* yBuf, char *yuyvBuf,int wid, int hig)
{
    if(!yBuf || !yuyvBuf)
    {
        return 0;
...
static int yuyv2rgb(unsigned char* yuyv,unsigned char * rgb, int width, int height)
{
...
int USB_CAM_Init(int wid, int hig){
    camera = camera_open("/dev/video0",wid,hig);
    camera_init(camera);
    camera_start(camera);
    timeout.tv_sec = 0;
    timeout.tv_usec = 100000;
    rgb = (unsigned char*)malloc(wid*hig*3);
    memset(rgb, 0, wid*hig*3);
...
int USB_CAM_Capture(void** buf){
    ...
    yuyv2rgb((unsigned char*)camera->head.start, rgb,camera->width,camera->height);
#if 0
    static int savedFile = 0;
    if(savedFile == 0)
    {
        savedFile = 1;
        saveFile("/sdcard/wissen/usb_rgb.yuv",rgb,camera->width*camera->height*3);
    }else{
        if(savedFile == 1)
        {
            savedFile = 2;
            saveFile("/sdcard/wissen/usb_yuv.yuv",(unsigned char*)camera->head.start,camera->width*camera->height);
        }
    }
#endif
    *buf = rgb;
